...
//------------------------------------------------------------------------- looks for "if" in the fucking Lexan object
        if (lexObj[counter] == "if")
        {
            ...
            else
            {
                ...
                stringstream stringToDigit(lexObj[counter + 1]);   // converting  string to long starting after the first token which is "if"

                stringToDigit >> left; // streaming it to the lhs 
            }
            if (isalpha(lexObj[counter + 3][0]))  // checking if t
            {
