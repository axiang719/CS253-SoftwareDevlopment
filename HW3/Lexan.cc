...
Lexan:: Lexan(const string &input) {
          ...
          while (getline(f,line)){                      //loop to put the input string into line 
     ...
     while(isdigit(line[len])){                    // scan a number
       if(len > 15){                               // checks to make sure digit length is less than 15. If more than 15, complain
  ...
  if(!len)                                     // number not found                               
      for(string tok : tokens)                 // look through tokens
          if(start_with(line, tok)){           // a match
            len = tok.size();                  // only length matters
             break;
        }
  
  if( line == ""){                            // if string contains a blank token, continue
      continue;
    }      
  
  if(!len){                                  // if token is bad complain
...
//-------------------------------------------------- function for checking if it is in range, if so return the nth index of lex vector    
  string Lexan::operator[](const size_t n) const{ 
    if( n > lex.size()-1 ){ // checking if n is greater than the size of the vector 
